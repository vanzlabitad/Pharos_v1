...
- **Unequal reporting windows.** Ingestion caps each drug at roughly its newest 10,000 report–reaction rows (`MAX_RECORDS_PER_DRUG` counts rows after each report is split into one row per reaction, not reports), so high-volume drugs span a shorter calendar window than low-volume ones and the comparator pool mixes periods. The Quarto report prints each drug's actual window. A fixed shared date window is V2.
